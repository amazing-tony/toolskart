/* =========================================================
   Amazing-Tools — Client-Side Document Converter Engine
...
(function () {
    ...
    async function convertDocxToPdf(buffer) {
        ...
        if (previewEngine && typeof previewEngine.renderAsync === 'function' && html2canvas) {
            ...
            try {
                ...
                // Ensure all embedded Word images are fully decoded before canvas rasterization
                const docxImages = Array.from(container.querySelectorAll('img'));
                if (docxImages.length > 0) {
                    await Promise.all(docxImages.map(img => {
        ...
        const mammothImages = Array.from(mContainer.querySelectorAll('img'));
        if (mammothImages.length > 0) {
            await Promise.all(mammothImages.map(img => {
                if (img.complete && img.naturalWidth > 0) return Promise.resolve();
                if (img.decode) {
                    return img.decode().catch(() => {});
                }
    ...
    async function convertPdfToDocx(buffer) {
        updateProgress(30, 'Parsing PDF pages and visual streams...');
        if (!window.pdfjsLib) {
        ...
        const loadingTask = pdfjsLib.getDocument({ data: new Uint8Array(buffer).slice() });
        const pdfDoc = await loadingTask.promise;
        const numPages = pdfDoc.numPages;

        const docxSections = [];
        const { Document, Paragraph, TextRun, ImageRun, AlignmentType } = window.docx;

        for (let i = 1; i <= numPages; i++) {
            updateProgress(30 + Math.round((i / numPages) * 50), `Processing page ${i} of ${numPages}...`);
            const page = await pdfDoc.getPage(i);
            const textContent = await page.getTextContent();
            const totalText = textContent.items.map(it => it.str).join('').trim();
            const hasText = totalText.length > 25;

            const pageParagraphs = [];

            // High-DPI canvas render for visual fidelity
            const vp = page.getViewport({ scale: 2.0 });
            const renderCanvas = document.createElement('canvas');
            renderCanvas.width = Math.round(vp.width);
            renderCanvas.height = Math.round(vp.height);
            ...
            if (!hasText) {
                // Scanned / Image-heavy page: embed high-DPI page render to ensure document is never blank
                const imgBlob = await new Promise(res => renderCanvas.toBlob(res, 'image/jpeg', 0.92));
                const imgBytes = new Uint8Array(await imgBlob.arrayBuffer());
                const unscaledVp = page.getViewport({ scale: 1.0 });
                const targetWidth = Math.min(500, Math.round(unscaledVp.width));
                const targetHeight = Math.round(targetWidth * (renderCanvas.height / renderCanvas.width));
                ...
                pageParagraphs.push(new Paragraph({
                    ...
                    alignment: AlignmentType ? AlignmentType.CENTER : undefined,
                    spacing: { after: 120 }
                }));
            } else {
                // Has text: extract structured text paragraphs
                let currentLineText = '';
                let lastY = null;
                ...
                try {
                    ...
                    for (let j = 0; j < ops.fnArray.length; j++) {
                        if (ops.fnArray[j] === pdfjsLib.OPS.paintImageXObject) {
                            ...
                            try {
                                const obj = await new Promise(r => page.objs.get(imgName, r));
                                if (obj && (obj.bitmap || obj.data) && obj.width >= 10 && obj.height >= 10) {
                                    ...
                                    let dispW = Math.min(480, obj.width);
                                    let dispH = Math.round(dispW * (obj.height / obj.width));
                                    if (dispH > 650) {
                                        dispW = Math.round(dispW * (650 / dispH));
                                        dispH = 650;
                                    }
                                    pageParagraphs.push(new Paragraph({
                                        children: [
                                ...
                                console.warn('Could not extract image object:', imgName, imgErr);
                            }
                        }
                    }
                } catch (opsErr) {
                    console.warn('Operator list check failed for page:', opsErr);
                }
            }

            // Fallback: If pageParagraphs ended up empty, guarantee the page is not blank
            if (pageParagraphs.length === 0) {
                ...
                pageParagraphs.push(new Paragraph({
                    children: [
                        new ImageRun({
                            data: imgBytes,
                            transformation: { width: 500, height: Math.round(500 * (renderCanvas.height / renderCanvas.width)) }
                        })
                    ],
                    alignment: AlignmentType ? AlignmentType.CENTER : undefined,
                    spacing: { after: 120 }
